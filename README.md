...
- GitHub 仓库首页只会把根目录的 **`README.md`**（或少数其它扩展名）当作 Markdown 渲染；若文件名叫 `README` 且无扩展名，页面会按纯文本显示，`![图片](...)` 会原样出现而不显示图。
- 若已使用 `README.md` 仍不显示截图，请确认 `images/` 下的 PNG 已提交并推送到远程，且路径大小写与仓库中一致。
